...
namespace arookas.DataSEQ {

	[Errand(Errand.DataSEQ)]
	class DataSEQErrand : IErrand {
				...
				switch (parameter.Name) {
					...
					case "-init-data-file": {
						if (parameter.Count == 0) {
							SMSAudioClass.WriteError("DataSEQ: bad -init-data-file parameter");
						}
					...
					case "-seq-data-file": {
						if (parameter.Count == 0) {
							SMSAudioClass.WriteError("DataSEQ: bad -seq-data-file parameter");
						}
		...
		public void ShowUsage() {
			SMSAudioClass.WriteMessage("USAGE: DataSEQ <action> [...]\n");
			SMSAudioClass.WriteMessage("\n");
			SMSAudioClass.WriteMessage("ACTIONS:\n");
			SMSAudioClass.WriteMessage("  -extract-seq    extract sequence data\n");
			SMSAudioClass.WriteMessage("  -replace-seq    replace sequence data\n");
			SMSAudioClass.WriteMessage("  -extract-ibnk   extract bank 'IBNK' data\n");
			SMSAudioClass.WriteMessage("  -replace-ibnk   replace bank 'IBNK' data\n");
			SMSAudioClass.WriteMessage("  -extract-wsys   extract wave bank 'WSYS' data\n");
			SMSAudioClass.WriteMessage("  -replace-wsys   replace wave bank 'WSYS' data\n");
			SMSAudioClass.WriteMessage("\n");
			SMSAudioClass.WriteMessage("OPTIONS:\n");
			SMSAudioClass.WriteMessage("  -init-data-file <file> [<output>]\n");
			SMSAudioClass.WriteMessage("    Specifies the init data file (AAF). When writing,\n");
			SMSAudioClass.WriteMessage("    <output> specifies the output file name.\n");
			SMSAudioClass.WriteMessage("  -seq-data-file <file> [<output>]\n");
			SMSAudioClass.WriteMessage("    Specifies the sequence data file (ARC). When writing,\n");
			SMSAudioClass.WriteMessage("    <output> specifies the output file name.\n");
			SMSAudioClass.WriteMessage("  -input <file>\n");
			SMSAudioClass.WriteMessage("    Specifies the data with which to replace.\n");
			SMSAudioClass.WriteMessage("  -output <file>\n");
			SMSAudioClass.WriteMessage("    Specifies the file to which to extract data.\n");
			SMSAudioClass.WriteMessage("  -target <value>\n");
			SMSAudioClass.WriteMessage("    Specifies which sequence, bank, or wave bank to\n");
			SMSAudioClass.WriteMessage("    extract or replace. For sequences, may be an index,\n");
			SMSAudioClass.WriteMessage("    filename, or ASN name. For banks and wave banks, only\n");
			SMSAudioClass.WriteMessage("    indices are supported.\n");
		}
		...
		void DoExtractSeq() {
			if (mAafInPath == null) {
				SMSAudioClass.WriteError("DataSEQ: missing -init-data-file parameter");
			}

			if (mArcInPath == null) {
				SMSAudioClass.WriteError("DataSEQ: missing -seq-data-file parameter");
			}

			if (mOutput == null) {
				SMSAudioClass.WriteError("DataSEQ: missing -output parameter");
			}

			if (mTarget == null) {
				SMSAudioClass.WriteError("DataSEQ: missing -target parameter");
			}

			int offset, size;

			using (Stream stream = SMSAudioClass.OpenFile(mAafInPath)) {
				SMSAudioClass.WriteMessage("Scanning AAF header...\n");
				aBinaryReader reader = new aBinaryReader(stream, Endianness.Big);

				if (!ReadAafHeader(reader, 4, 0, out offset, out size)) {
					SMSAudioClass.WriteError("DataSEQ: failed to find sequence info data");
				}

				reader.Goto(offset);

				if (reader.ReadS32() != 0x42415243) { // 'BARC'
					SMSAudioClass.WriteError("DataSEQ: could not find 'BARC' header");
				}
				...
				SMSAudioClass.WriteMessage("Found sequence info data (0x{0:X6}, 0x{1:X6}), {2} sequence(s)\n", offset, size, count);
				SMSAudioClass.WriteMessage("Scanning sequence list...\n");

				if (!ReadBarcHeader(reader, mTarget, count, out index, out offset, out size)) {
					SMSAudioClass.WriteError("DataSEQ: could not find sequence {0}", mTarget);
				}

				SMSAudioClass.WriteMessage("Found sequence {0} (0x{1:X6}, 0x{2:X6})\n", index, offset, size);
			}

			SMSAudioClass.WriteMessage("Extracting sequence data...\n");

			using (Stream stream = SMSAudioClass.OpenFile(mArcInPath)) {
		...
		void DoReplaceSeq() {
			...
			if (mInput == null) {
				SMSAudioClass.WriteError("DataSEQ: missing -input parameter");
			}
			...
			using (Stream input = SMSAudioClass.OpenFile(mAafInPath)) {
				SMSAudioClass.WriteMessage("Scanning AAF header...\n");
				aBinaryReader reader = new aBinaryReader(input, Endianness.Big);
				int offset, size;

				if (!ReadAafHeader(reader, 4, 0, out offset, out size)) {
					SMSAudioClass.WriteError("DataSEQ: failed to find sequence info block");
				}
				...
				using (Stream arc_stream = SMSAudioClass.CreateFile(mArcOutPath))
				using (MemoryStream barc_stream = new MemoryStream(barc_data, true)) {
					SMSAudioClass.WriteMessage("Writing new sequence data...\n");
				...
				using (Stream output = SMSAudioClass.CreateFile(mAafOutPath)) {
					SMSAudioClass.WriteMessage("Writing new AAF file...\n");
					aBinaryWriter writer = new aBinaryWriter(output, Endianness.Big);

					if (!WriteAafHeader(reader, writer, 4, 0, barc_data)) {
						SMSAudioClass.WriteError("DataSEQ: failed to write aaf file");
					}
				}
			}
		}
		...
		void DoExtractIbnk() {
			...
			if (!Int32.TryParse(mTarget, out index)) {
				SMSAudioClass.WriteError("DataSEQ: bad target {0}", mTarget);
			}
			...
			using (Stream stream = SMSAudioClass.OpenFile(mAafInPath)) {
				...
				if (!ReadAafHeader(reader, 2, index, out offset, out size)) {
					SMSAudioClass.WriteError("DataSEQ: failed to find bank block\n");
				}

				reader.Goto(offset);

				if (reader.ReadS32() != 0x49424E4B) { // 'IBNK'
					SMSAudioClass.WriteError("DataSEQ: could not find 'IBNK' header");
				}

				SMSAudioClass.WriteMessage("Found bank data {0} (0x{1:X6}, 0x{2:X6})\n", index, offset, size);
				SMSAudioClass.WriteMessage("Extracting bank data...\n");
				reader.Goto(offset);
				WriteFileData(mOutput, reader.Read8s(size));
			}
		}
		...
		void DoReplaceIbnk() {
			...
			SMSAudioClass.WriteMessage("Loading input file data...\n");
		...
		void DoExtractWsys() {
			...
			using (Stream stream = SMSAudioClass.OpenFile(mAafInPath)) {
				...
				if (!ReadAafHeader(reader, 3, index, out offset, out size)) {
					SMSAudioClass.WriteError("DataSEQ: failed to find wave bank data\n");
				}

				reader.Goto(offset);

				if (reader.ReadS32() != 0x57535953) { // 'WSYS'
					SMSAudioClass.WriteError("DataSEQ: could not find 'WSYS' header");
				}
				
				SMSAudioClass.WriteMessage("Found wave bank data {0} (0x{1:X6}, 0x{2:X6})\n", index, offset, size);
				SMSAudioClass.WriteMessage("Extracting wave bank data...\n");
				reader.Goto(offset);
				WriteFileData(mOutput, reader.Read8s(size));
			}
		}
		...
		static bool WriteAafHeader(aBinaryReader reader, aBinaryWriter writer, int id, int index, byte[] data) {
			...
			SMSAudioClass.WriteMessage("Entry size difference: {0} byte(s)\n", difference);
			int danger_level = 0;
			...
			while ((section = reader.ReadS32()) != 0) {
				if (danger_level++ > 1000) {
					SMSAudioClass.WriteError("DataSEQ: malformed AAF file (endless loop detected)");
				}
		...
		static byte[] ReadFileData(string path) {
			try {
				...
				SMSAudioClass.WriteError("DataSEQ: failed to read file {0}", path);
			}
		...
		static void WriteFileData(string path, byte[] data) {
			try {
				...
				SMSAudioClass.WriteError("DataSEQ: failed to write file {0}", path);
			}
		}
