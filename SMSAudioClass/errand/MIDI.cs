...
namespace arookas.MIDI {

	[Errand(Errand.MIDI)]
	class MIDIErrand : MidiReader, IErrand {
		...
		public void LoadParams(string[] arguments) {
			...
			if (parameter == null) {
				SMSAudioClass.WriteError("MIDI: missing -input parameter.");
			} else if (parameter.Count == 0) {
				SMSAudioClass.WriteError("MIDI: missing argument for -input parameter.");
			}
			...
			if (parameter == null) {
				SMSAudioClass.WriteError("MIDI: missing -output parameter.");
			} else if (parameter.Count == 0) {
				SMSAudioClass.WriteError("MIDI: missing argument for -output parameter.");
			}
			...
			if (parameter != null) {
				...
				if (parameter.Count >= 1) {
					if (!Int64.TryParse(parameter[0], NumberStyles.None, null, out mLoop)) {
						SMSAudioClass.WriteError("MIDI: bad loop value '{0}'.", parameter[0]);
					}
				}
				...
				if (parameter.Count >= 2) {
					switch (parameter[1].ToLowerInvariant()) {
						...
						default: {
							SMSAudioClass.WriteError("MIDI: bad time unit '{0}'.", parameter[1]);
							break;
						}
					}
				}
		...
		public void ShowUsage() {
			SMSAudioClass.WriteMessage("USAGE: MIDI -input <file> -output <file> [...]\n");
			SMSAudioClass.WriteMessage("\n");
			SMSAudioClass.WriteMessage("OPTIONS:\n");
			SMSAudioClass.WriteMessage("  -loop [<amount> [<unit>]]\n");
			SMSAudioClass.WriteMessage("    Specifies the song should loop from the end. <amount>\n");
			SMSAudioClass.WriteMessage("    and <unit> specify the beginning of the loop; if omitted,\n");
			SMSAudioClass.WriteMessage("    the entire song will loop front-to-back.\n");
		}
		...
		public void Perform() {
			using (var instream = SMSAudioClass.OpenFile(mInput)) {
				...
				if (Format == 2) {
					SMSAudioClass.WriteError("MIDI: format-2 MIDIs are not supported.");
				}

				SMSAudioClass.WriteMessage("Format-{0} MIDI {1} division {2} tracks\n", Format, Division, TrackCount);
				...
				if (mLoop >= 0) {
					...
					SMSAudioClass.WriteMessage("Loop enabled: {0} ticks\n", mLoop, Division);
				}
			...
			using (var outstream = SMSAudioClass.CreateFile(mOutput)) {
				...
				mWriter.WriteLine();
				mWriter.WriteLine("# this file was generated by SMSAudioClass");
		...
		public bool AddNoteOn(long time, int key, int velocity) {
			for (var i = 0; i < mActiveNotes.Length; ++i) {
				if (mActiveNotes[i] < 0 || mActiveNotes[i] == key) {
					mActiveNotes[i] = key;
					AddEvent(time, "noteon {0}, {1}, {2}", SMSAudioClass.ConvertKey(key), velocity, (i + 1));
					return true;
				}
			}
			return false;
