...
namespace arookas {
	...
	static partial class SMSAudioClass {
		...
		public static void WriteMessage(string message) { SMSAudioClass.WriteMessage("{0}", message); }
		...
		public static void WriteWarning(string message) { SMSAudioClass.WriteWarning("{0}", message); }
		...
		public static void WriteError(string message) { SMSAudioClass.WriteError("{0}", message); }
		...
		public static Stream CreateFile(string filename, bool fatal = true) {
			if (filename == null) {
				if (fatal) {
					SMSAudioClass.WriteError("SYSTEM: null filename while creating file.");
				} else {
					SMSAudioClass.WriteWarning("SYSTEM: null filename while creating file\n");
				}
			...
			try {
				...
				if (fatal) {
					SMSAudioClass.WriteError("SYSTEM: failed to create file '{0}'.", filename);
				} else {
					SMSAudioClass.WriteWarning("SYSTEM: failed to create file '{0}'\n", filename);
				}
		...
		public static Stream OpenFile(string filename, bool fatal = true) {
			if (filename == null) {
				if (fatal) {
					SMSAudioClass.WriteError("SYSTEM: null filename while opening file.");
				} else {
					SMSAudioClass.WriteWarning("SYSTEM: null filename while opening file\n");
				}
			...
			try {
				...
				if (fatal) {
					SMSAudioClass.WriteError("SYSTEM: failed to open file '{0}'.", filename);
				} else {
					SMSAudioClass.WriteWarning("SYSTEM: failed to open file '{0}'\n", filename);
				}
		...
		public static string GetRelativePath(string basepath, string path) {
			...
			var basepaths = SMSAudioClass.GetPathComponents(basepath);
			var paths = SMSAudioClass.GetPathComponents(path);
