	...
	abstract class MidiReader {
		...
		protected void LoadMidi(Stream stream) {
			...
			while (!mReader.IsAtEndOfStream) {
				if (mReader.BytesRemaining < 8) {
					SMSAudioClass.WriteWarning("MIDI: incomplete chunk at file end.\n");
					break;
				}
			...
			if (!mthd) {
				SMSAudioClass.WriteError("MIDI: missing header chunk.");
			}

			if (tracks != mTrackCount) {
				SMSAudioClass.WriteWarning("MIDI: track count mismatch (header says {0}, found {1}).\n", mTrackCount, tracks);
				mTrackCount = tracks;
			}

			if (mFormat == 0 && mTrackCount != 1) {
				SMSAudioClass.WriteWarning("MIDI: format-0 requires a single track.");
			}
		}
		void LoadMThd(int size) {
			if (size < 6) {
				SMSAudioClass.WriteError("MIDI: bad header size '{0}'.", size);
			}

			mFormat = mReader.Read16();

			if (mFormat > 2) {
				SMSAudioClass.WriteError("MIDI: unknown format '{0}'.", mFormat);
			}
			...
			if ((mDivision & 0x8000) != 0) {
				SMSAudioClass.WriteError("MIDI: SMTPE divisions are not supported.");
			}
		}
		...
		protected bool ReadEvent(out EventInfo info) {
			...
			if (mReader.Position >= (mCurrentTrack.start + mCurrentTrack.size)) {
				if (!mEndOfTrack) {
					SMSAudioClass.WriteWarning("MIDI: non-terminated track.\n");
				}
				return false;
			} else if (mEndOfTrack) {
				SMSAudioClass.WriteError("MIDI: last event in track is not EOT marker.");
			}
			...
			if ((status & 0x80) == 0) {
				if (mRunningStatus < 0) {
					SMSAudioClass.WriteError("MIDI: invalid running status.");
				}
			...
			if (status < 0xF0) {
				...
				SMSAudioClass.WriteError("MIDI: unknown status '{0:X2}'.", status);
			}
