			...
			using (var instream = mareep.OpenFile(mInputFile)) {
				...
				using (var outstream = mareep.CreateFile(mOutputFile)) {
					switch (mInputFormat) {
						...
						case IOFormat.LE: chain = new BinaryBankDeserializer(CreateLittleBinaryInput(instream), inputName); break;
						case IOFormat.BE: chain = new BinaryBankDeserializer(CreateBigBinaryInput(instream), inputName); break;
